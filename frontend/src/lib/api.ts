...
import type { Region, NasaDay, EvaRow, PredictionPayload, PredictionResult } from "@/types";
...
export const api = {
  ...
  eva: {
    getData: (departamento: string, cultivo = "MAIZ", limit = 20) =>
      fetchJson<{ success: boolean; data: EvaRow[]; total_registros: number }>(
        `${AI_URL}/eva/data?departamento=${departamento}&cultivo=${cultivo}&limit=${limit}`
      ),
  },
  predict: {
